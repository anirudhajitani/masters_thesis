...
if __name__ == "__main__":
    ...
    replay_buffer = utils.ReplayBuffer(
        state_dim, False, None, 10000, 10000, 'cpu')
    # Loads replay buffer and does preprocessing of next state
    replay_buffer.load(args.folder, args.replay_buffer)
    
    if args.algo == 3:
        model = structured_learning.structured_learning(
            False, num_actions, state_dim, device, 0)
        if int(args.fc) - 1 > 0:
            # If not the first run, load previous buffers and set these values for SALMUT
            run = int(args.run)
            thres_vec = np.load(
    ...
    exit(1)
    """ 
    # We are not processing other RL algos for TESTBED
    for j in range(0, 10):
        print("RANDOM SEED ", j)
        lambd = []
        N = []
